...
#define KERNEL_CMDLINE_CONTINUATION_LEN \
	(sizeof(KERNEL_CMDLINE_CONTINUATION) - 1)

#define MIN_CMDLINE_LOG_WRAP_IDEAL_LEN \
	(KERNEL_CMDLINE_PREFIX_LEN + KERNEL_CMDLINE_CONTINUATION_LEN)
#define CMDLINE_LOG_WRAP_IDEAL_LEN                                            \
	(CONFIG_CMDLINE_LOG_WRAP_IDEAL_LEN > MIN_CMDLINE_LOG_WRAP_IDEAL_LEN ? \
		 CONFIG_CMDLINE_LOG_WRAP_IDEAL_LEN :                          \
		 MIN_CMDLINE_LOG_WRAP_IDEAL_LEN)

#define IDEAL_CMDLINE_LEN \
	(CMDLINE_LOG_WRAP_IDEAL_LEN - KERNEL_CMDLINE_PREFIX_LEN)
#define IDEAL_CMDLINE_SPLIT_LEN \
	(IDEAL_CMDLINE_LEN - KERNEL_CMDLINE_CONTINUATION_LEN)
...
static void __init print_kernel_cmdline(const char *cmdline)
{
	...
	while (len > IDEAL_CMDLINE_LEN) {
		...
		while (true) {
			...
			if (!cutoff ||
			    cutoff - cmdline > IDEAL_CMDLINE_SPLIT_LEN)
				break;
			prev_cutoff = cutoff;
		}
		if (prev_cutoff)
		...
		if (to_print)
			pr_notice("%s%.*s%s\n", KERNEL_CMDLINE_PREFIX, to_print,
				  cmdline, KERNEL_CMDLINE_CONTINUATION);
	...
	do {
		...
		if (str_entry) {
			...
			entry = memblock_alloc_or_panic(sizeof(*entry),
							SMP_CACHE_BYTES);
			entry->buf = memblock_alloc_or_panic(
				strlen(str_entry) + 1, SMP_CACHE_BYTES);
