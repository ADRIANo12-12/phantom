...
{
	pr_info_once(
		"Kernel: The 'panic_print' parameter is now deprecated. Please use 'panic_sys_info' and 'panic_console_replay' instead.\n");
}
 ...
 */
static int proc_taint(const struct ctl_table *table, int write, void *buffer,
 ...
 */
__printf(1, 0) static bool panic_try_force_cpu(const char *fmt, va_list args)
{
...
#else
__printf(1, 0) static inline bool panic_try_force_cpu(const char *fmt,
						      va_list args)
{
	return false;
}
...
static void phantom_panic_screen(const char *reason)
{
	pr_emerg("\033[2J\033[H");

	pr_emerg("\n\n");
	pr_emerg("                 P H A N T O M   O S\n");
	pr_emerg("\n\n");

	pr_emerg("          Oops! Your system encountered an error\n");
	pr_emerg("          that couldn't be fixed!\n");
	pr_emerg("\n");

	pr_emerg("          We're finding the reason about what\n");
	pr_emerg("          started this problem and making a\n");
	pr_emerg("          dumpstack file with all the information\n");
	pr_emerg("          about this error.\n");
	pr_emerg("\n");
	...
	pr_emerg("\n");
	pr_emerg("\n");
	pr_info("This is a custom phantom os panic system.\n\n");
	pr_emerg("             .--.\n");
	pr_emerg("            |X_X |\n");
	pr_emerg("            |:_/ |\n");
	pr_emerg("           //   \\ \\\n");
	...
	pr_emerg("\n");
	pr_emerg("          YOU CAN SAFELY REBOOT\n");
	pr_emerg("\n");

	pr_emerg("          Phantom OS will restart automatically.\n");
}
...
void vpanic(const char *fmt, va_list args)
{
	...
	if (atomic_read(&panic_redirect_cpu) != PANIC_CPU_INVALID &&
		...
		pr_emerg(
			"panic: Redirected from CPU %d, skipping stack dump.\n",
	    ...
	    struct pt_regs *regs, struct warn_args *args)
{
	...
	if (file) {
		pr_warn("WARNING: %s:%d at %pS, CPU#%d: %s/%d\n", file, line,
			caller, raw_smp_processor_id(), current->comm,
			current->pid);
	} else {
