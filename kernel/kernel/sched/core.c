...
void raw_spin_rq_lock_nested(struct rq *rq, int subclass) __context_unsafe()
{
	raw_spinlock_t *lock;
...
bool raw_spin_rq_trylock(struct rq *rq) __context_unsafe()
{
	raw_spinlock_t *lock;
	bool ret;
...
void hrtick_start(struct rq *rq, u64 delay)
{
	...
	if (rq == this_rq())
		hrtimer_start(&rq->hrtick_timer, rq->hrtick_time,
...
static void wake_up_idle_cpu(int cpu)
{
	 ...
	 * much benefits.
	 */
	if (set_nr_and_not_polling(task_thread_info(rq->idle),
				   TIF_NEED_RESCHED))
		smp_send_reschedule(cpu);
	else
		trace_sched_wake_idle_without_ipi(cpu);
		       ...
		       unsigned long task_state)
{
	...
	p->sched_contributes_to_load = (task_state & TASK_UNINTERRUPTIBLE) &&
...
static void do_set_cpus_allowed(struct task_struct *p,
...
static inline void mm_update_cpus_allowed(struct mm_struct *mm,
					  const cpumask_t *affmask);
 ...
 */
void set_cpus_allowed_force(struct task_struct *p,
...
static int migrate_swap_stop(void *data)
{
	...
	guard(double_raw_spinlock)
		(&arg->src_task->pi_lock, &arg->dst_task->pi_lock);
	guard(double_rq_lock)
		(src_rq, dst_rq);
...
static inline void prepare_lock_switch(struct rq *rq, struct task_struct *next,
				       struct rq_flags *rf)
	__releases(__rq_lockp(rq)) __acquires(__rq_lockp(this_rq()))
{
	/*
...
static inline void finish_lock_switch(struct rq *rq) __releases(__rq_lockp(rq))
{
	/*
	...
	__must_hold(__rq_lockp(rq))
{
	 ...
	 */
	if (likely(!sched_class_above(rq->donor->sched_class,
				      &fair_sched_class) &&
...
static void sched_core_balance(struct rq *rq) __must_hold(__rq_lockp(rq))
{
	struct sched_domain *sd;
	int cpu = cpu_of(rq);
...
find_proxy_task(struct rq *rq, struct task_struct *donor, struct rq_flags *rf)
{
	WARN_ONCE(
		1,
		"This should never be called in the !SCHED_PROXY_EXEC case\n");
	return donor;
}
...
void sched_show_task(struct task_struct *p)
{
	...
	pr_cont(" stack:%-5lu pid:%-5d tgid:%-5d ppid:%-6d task_flags:0x%04x flags:0x%08lx\n",
		free, task_pid_nr(p), task_tgid_nr(p), ppid, p->flags,
 ...
 */
static void balance_push(struct rq *rq) __must_hold(__rq_lockp(rq))
{
...
static int tg_set_cfs_bandwidth(struct task_group *tg, u64 period_us,
...
static void tg_bandwidth(struct task_group *tg, u64 *period_us_p,
			 u64 *quota_us_p, u64 *burst_us_p)
{
#ifdef CONFIG_CFS_BANDWIDTH
	if (period_us_p)
...
static int tg_set_bandwidth(struct task_group *tg, u64 period_us, u64 quota_us,
			    u64 burst_us)
{
	...
	if (quota_us != RUNTIME_INF &&
	    (burst_us > quota_us || burst_us + quota_us > max_bw_runtime_us))
		return -EINVAL;
...
static inline void mm_cid_complete_transit(struct mm_struct *mm,
					   unsigned int mode)
{
	/*
...
static inline void mm_cid_transit_to_task(struct task_struct *t,
...
static inline void mm_cid_transit_to_cpu(struct task_struct *t,
					 struct mm_cid_pcpu *pcp)
{
...
static void mm_cid_fixup_task_to_cpu(struct task_struct *t,
				     struct mm_struct *mm)
{
	/* Remote access to mm::mm_cid::pcpu requires rq_lock */
	guard(task_rq_lock)
	...
	if (cid_on_task(t->mm_cid.cid)) {
		...
		if (task_rq(t)->curr == t)
			mm_cid_transit_to_cpu(t, per_cpu_ptr(mm->mm_cid.pcpu,
							     task_cpu(t)));
		else
	...
	scoped_guard(mutex, &mm->mm_cid.mutex) {
		...
		if (likely(mm->mm_cid.users > 1)) {
			scoped_guard(raw_spinlock_irq, &mm->mm_cid.lock) {
				 ...
				 */
				mm_drop_cid_on_cpu(
					mm, this_cpu_ptr(mm->mm_cid.pcpu));
			}
			mm_cid_fixup_cpus_to_tasks(mm);
			return;
		}
		...
		scoped_guard(raw_spinlock_irq, &mm->mm_cid.lock) {
			...
			if (t == current)
				mm_cid_transit_to_task(
					t, this_cpu_ptr(mm->mm_cid.pcpu));
...
{
	struct mm_struct *mm =
		container_of(work, struct mm_struct, mm_cid.work);
...
{
	struct mm_struct *mm =
		container_of(work, struct mm_struct, mm_cid.irq_work);
...
void sched_change_end(struct sched_change_ctx *ctx)
{
	 ...
	 */
	WARN_ON_ONCE(p->sched_class != ctx->class &&
		     !(ctx->flags & ENQUEUE_CLASS));
